package org.usfirst.frc.owatonnarobotics.input;
...
public class XboxController extends Joystick {
  ...
  /* Default Values */
  private static final double DEFAULT_THUMBSTICK_DEADZONE = 0.1; // Jiggle room
                                                                 // for the
                                                                 // thumbsticks
  ...
  public static enum DPAD {
    ...
    public static DPAD getEnum(int angle) {
      ...
      angle %= 360;
      angle = Math.round(angle / 45) * 45; // May have rounding errors. Due to
                                           // rounding errors.
      ...
      // throw new UnsupportedOperationException("Integer supplied (" + angle +
      // ") is not a possible value of this enum.");
      System.out.println("[XboxController.DPAD.getEnum()] Angle supplied ("
          + angle + ") has no related DPad direction");
      return DPAD.UP;
    }
  }
  ...
  public static class Thumbstick extends Button {
     ...
     */
    private double rawX() {
      final double rawInput = this.parent.getRawAxis(this.xAxisID);

      return createDeadZone(rawInput, this.xDeadZone);
    }
     ...
     */
    private double rawY() {
      final double rawInput = -this.parent.getRawAxis(this.yAxisID); // -Y was
                                                                     // up on
                                                                     // our
                                                                     // thumbsticks.
                                                                     // Consider
      ...
      return createDeadZone(rawInput, this.yDeadZone);
    }
    ...
    @Override
    public boolean get() {
      return this.parent.getRawButton(this.pressedID);
    }
  ...
  public static class Trigger extends Button {
    ...
    public double getX() {
      ...
      if (this.hand == HAND.LEFT) {
        rawInput = this.parent.getRawAxis(LEFT_TRIGGER_AXIS_ID);
      } else {
        rawInput = this.parent.getRawAxis(RIGHT_TRIGGER_AXIS_ID);
      }
    ...
    public double getY() {
      return getX(); // Triggers have one dimensional movement. Use getX()
                     // instead
    }
  ...
  private static double createDeadZone(double input, double deadZoneSize) {
    ...
    adjusted = adjusted < 0 ? 0 : adjusted; // if the new input is negative,
                                            // make it zero
    adjusted = adjusted / (1 - deadZoneSizeClamp); // Adjust the adjustment so
                                                   // it can max at 1
   ...
   */
  public void setDeadZone(double number) {
    this.leftStick.setDeadZone(number);
    this.rightStick.setDeadZone(number);
  }
}
